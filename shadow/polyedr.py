...
class Facet:
    ...
    def angle_with_horizontal(self):
        ...
        return acos(abs(normal.dot(Polyedr.V)) / normal.length())

    def perimeter(self):
        perim = 0
        for i in range(len(self.vertexes)):
            perim += self.vertexes[i].range(self.vertexes[i - 1])
        return perim
...
class Polyedr:
    ...
    def shadowfication(self):  # pragma: no cover
        for e in self.edges:
    ...
    def perimeter(self):
        ...
        perimeter = 0
        for f in self.facets:
            if f.angle_with_horizontal() <= pi/7 and\
                    not (-0.5 * self.c <= f.center().x <= 0.5 * self.c
                         and -0.5 * self.c <= f.center().y <= 0.5 * self.c):
                vision_status = False
                for i in range(len(f.vertexes) - 1):
                    beginpoint = f.vertexes[i]
                    endpoint = f.vertexes[i + 1]
                    for e in self.edges:
                        if e.beg == beginpoint and e.fin == endpoint:
                            if e.vision:
                                vision_status = True
                                break
                    if vision_status:
